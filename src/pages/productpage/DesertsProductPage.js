...
const CoastalProductPage = () => {
    const { desertsProductId } = useParams();
    const { desertsProducts } = ProductState();

    const thisproduct = desertsProducts.find((prod) => prod.title === desertsProductId);
...
export default CoastalProductPage;
