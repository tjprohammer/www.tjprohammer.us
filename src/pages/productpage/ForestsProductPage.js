...
const ForestProductPage = () => {
    const { lushrainforestsId } = useParams();
    const { rainforestsProducts } = ProductState();

    const thisproduct = rainforestsProducts.find((prod) => prod.title === lushrainforestsId);
...
export default ForestProductPage;
